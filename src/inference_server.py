...
class TTSServer:
    ...
    def __init__(
        ...
        self._ultra_low = low_vram and torch.cuda.get_device_properties(0).total_memory < 10 * 1024**3
    ...
    def _gemma_to_cpu(self) -> None:
        ...
        if pe is not None and hasattr(pe, '_warm_text_encoder') and pe._warm_text_encoder is not None:
            ...
            msg = f"Gemma → CPU: {time.time() - t0:.1f}s"
            if self._ultra_low:
                t0 = time.time()
                self._velocity_model.to(self.device)
                self._audio_decoder.to(self.device)
                msg += f", DiT+VAE → GPU: {time.time() - t0:.1f}s"
            log.info(f"{msg}, GPU 显存: {torch.cuda.memory_allocated(self.device) / 1e9:.1f}GB")
    ...
    def _gemma_to_gpu(self) -> None:
        ...
        if self._ultra_low:
            ...
            torch.cuda.empty_cache()
            log.info(f"DiT+VAE → CPU (腾显存给 Gemma): {time.time() - t0:.1f}s")
        pe = self._prompt_encoder
        if pe is not None and hasattr(pe, '_warm_text_encoder') and pe._warm_text_encoder is not None:
            t0 = time.time()
            pe._warm_text_encoder.to(self.device)
            log.info(f"Gemma → GPU: {time.time() - t0:.1f}s")
